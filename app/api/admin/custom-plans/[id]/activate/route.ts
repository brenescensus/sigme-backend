...
// // FIXED: Proper null handling for TypeScript types
...
//       //  FIX: Helper function to convert -1 to 999999, null to undefined
//       const convertLimit = (value: number | null): number | undefined => {
...
//             //  FIX: Use convertLimit to handle null properly
//             subscribers_limit: convertLimit(customPlan.subscribers_limit),
//             websites_limit: convertLimit(customPlan.websites_limit) ?? 1, // Default to 1 if undefined
//             notifications_limit: convertLimit(customPlan.notifications_limit) ?? 10000,
//             recurring_limit: convertLimit(customPlan.journeys_limit),
            
//             subscription_starts_at: customPlan.contract_start_date || new Date().toISOString(),
//             subscription_ends_at: customPlan.contract_end_date || undefined,
...
//       return NextResponse.json({
//         success: true,
//         message: `Custom plan "${customPlan.plan_name}" activated and assigned to ${targetUser.email}`,
//         plan: {
//           id: customPlan.id,
//           plan_code: customPlan.plan_code,
//           plan_name: customPlan.plan_name,
...
//       .from('user_subscriptions')
//       .select('subscribers_used, websites_used, notifications_used, recurring_used')
//       .eq('user_id', userId)
//       .single();
...
// app/api/admin/custom-plans/[id]/activate/route.ts
//  FIXED: TypeScript-safe with proper null handling
  ...
  return withSuperAdmin(async (req, user) => {
    ...
    try {
      ...
      const updateData = {
        ...
        //  Use null coalescing to provide defaults while keeping type safety
        websites_limit: convertLimit(customPlan.websites_limit) ?? 1,
        notifications_limit: convertLimit(customPlan.notifications_limit) ?? 10000,
