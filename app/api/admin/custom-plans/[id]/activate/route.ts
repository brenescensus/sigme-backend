...
//
//   // Upsert user subscription pointing to this custom plan
...
//     status: 'active',
//     subscription_starts_at: plan.contract_start_date,
//     subscription_ends_at: plan.contract_end_date,
//   }, { onConflict: 'user_id' });
//
//   // Audit
//   await supabase.from('custom_plan_audit_log').insert({
  ...
  return withSuperAdmin(async (req, user) => {
    ...
    try {
      ...
      await logAdminActivity(
        ...
        'custom_plan',
        planId,
        { plan_code: plan.plan_code, plan_name: plan.plan_name, user_id: plan.user_id }
      );
