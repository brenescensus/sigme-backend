...
// // // // ============================================
// // // // app/api/campaigns/[id]/send/route.ts
// // // // FIXED - Properly passes subscriber IDs
// // // // ============================================
...
// // //       console.log('[Campaign Send] Found subscribers:', subscribers.length);
// // //       console.log('[Campaign Send] Subscriber IDs:', subscribers.map(s => s.id));

// // //       // ✅ FIX: Extract subscriber IDs to send to
// // //       const subscriberIds = subscribers.map(sub => sub.id);

// // //       // Prepare notification payload with subscriber IDs
// // //       const notificationPayload = {
// // //         websiteId: campaign.website_id,
// // //         campaignId: campaign.id,
// // //         targetSubscriberIds: subscriberIds, // ✅ ADD THIS!
// // //         notification: {
// // //           title: campaign.title,
// // //           body: campaign.body,
// // //           icon: campaign.icon_url || undefined,
...
//       // Update campaign with final stats
//       console.log('[Campaign Send] Updating campaign stats:', {
...
//         .from('campaigns')
//         .update({
//           sent_count: sendResult.sent || 0,           // Total attempted
//           delivered_count: sendResult.delivered || 0, // Successfully delivered
//           failed_count: sendResult.failed || 0,       // Failed
//           status: 'sent',
//           sent_at: new Date().toISOString(),
//           updated_at: new Date().toISOString(),
//         })
...
// app/api/campaigns/[id]/send/route.ts
// FIXED - Uses correct campaign status
  ...
  async (req: NextRequest, user: AuthUser, context: any) => {
    try {
      ...
      if (subsError) {
        ...
        await supabase
          .from('campaigns')
          .update({ status: 'draft' }) // ✅ Use valid status
          .eq('id', campaignId);
      ...
      // ✅ FIX: Use "active" instead of "sent" - valid status in your database
      const { error: updateError } = await supabase
        .from('campaigns')
        .update({
          sent_count: sendResult.sent || 0,
          delivered_count: sendResult.delivered || 0,
          failed_count: sendResult.failed || 0,
          status: 'active', // ✅ FIXED: Use valid status from database
          sent_at: new Date().toISOString(),
          updated_at: new Date().toISOString(),
        })
        .eq('id', campaignId);
      ...
      if (updateError) {
        ...
        // The notifications were already sent successfully
      }
      ...
      // Try to update campaign status to draft (failed state)
      try {
        const params = await context.params;
        const supabase = await getAuthenticatedClient(req);
        await supabase
